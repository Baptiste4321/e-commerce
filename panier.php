...
FROM Produit p 
JOIN Produit_dans_panier pdp ON p.ID_produit = pdp.ID_produit 
...
$panier = array(); // Créer un tableau pour stocker les produits du panier
...
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    if (array_key_exists($row["ID_produit"], $panier)) {
        // Si oui, augmenter la quantité du produit existant
        $panier[$row["ID_produit"]]["Quantite"] += $row["Quantite"];
    } else {
        // Sinon, ajouter le produit au panier
        $panier[$row["ID_produit"]] = $row;
...
foreach ($panier as $produit) {
    ...
    <form action="#" method="post">
        ...
        <br>
        <?php echo $produit["ID_produit_dans_panier"] ?>
        <br>
        Quantité: <?php echo $produit["Quantite"] ?>
        <br>
        <input type="hidden" value="<?php echo $produit["ID_produit_dans_panier"]; ?>" name="suprtache">
